...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _selectFolder(BuildContext context) async {
    String? path = await FilePicker.platform
        .getDirectoryPath(dialogTitle: "Select Images Folder");
    if (path != null) {
      _setSelectedFilesAndFolder(path);
    }
  }

  void _setSelectedFilesAndFolder(String path) {
      ...
      controls = <Widget>[
        DropTarget(
          onDragDone: (details) {
            ...
            _setSelectedFilesAndFolder(details.files.first.path);
          },
          child: Container(
            margin: const EdgeInsets.only(bottom: 4.0),
            child: OutlinedButton(
